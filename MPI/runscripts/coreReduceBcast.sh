...
for datasets in 1966080 62914560
	...
	for processos in 2 4 6 8 10 12 16 20 24 30 32
		do
		mpirun --map-by core -np $processos -report-bindings --mca btl self,sm,tcp bin/kmeans_mpi par 8 10 $datasets "datasets/input"$datasets".data" > "out/Core/ReduceBcast/"$datasets"_"$processos".txt"
	done
done
