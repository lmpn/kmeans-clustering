...
for datasets in 62914560
	...
	for processos in 2 4 8 12 16 24 32
		do
		mpirun --map-by ppr:1:core -np $processos -report-bindings --mca btl self,sm,tcp bin/rbcore par 8 10 $datasets "datasets/input"$datasets".data" > "out/Core - ReduceBcast_"$processos"_"$datasets
	done
done
